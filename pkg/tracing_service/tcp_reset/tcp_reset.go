...
func EnrichWithTcpResetEvent(base *schema.Base, event *tracing_service.BpfTcpResetEvent) error {
	if base == nil {
		return nil
	}
	...
	// RST during SYN_SENT means ECONNREFUSED; on an established connection it
	// is ECONNRESET. Surface that distinction so consumers don't have to infer.
	errnoCode := "ECONNRESET"
	if event.State == 2 { // SYN_SENT
	...
	base.Message = fmt.Sprintf("%s -> %s tcp rst_received %s %s", srcAddr, dstAddr, stateName, errnoCode)
...
func Run(ctx context.Context, program *ebpf.Program, ebpfMap *ebpf.Map) iter.Seq2[*tracing_service.EventResult, error] {
	return func(yield func(*tracing_service.EventResult, error) bool) {
		...
		err := tracing.RunTracingMapReceiver(
			...
			program,
			ebpfMap,
			func(event *tracing_service.BpfTcpResetEvent) {
				...
				base := &schema.Base{
					Event: &schema.Event{
						...
						Action:   "tcp_reset",
						Module:   "tracing",
						Reason:   "A TCP RST segment was received for an existing socket.",
						Dataset:  "tracing.tcp_reset",
					},
				}

				EnrichWithTcpResetEvent(base, event)
